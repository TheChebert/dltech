...
export function SignInForm() {
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_20px_65px_rgba(11,22,38,.09)]">
      ...
      <label className="mt-6 grid gap-2 text-sm font-medium text-slate-800">
        Email address
        <input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" required maxLength={254} className="h-12 rounded-lg border border-slate-300 px-3 text-base focus:border-blue-500 focus:outline-none focus:ring-3 focus:ring-blue-100" />
      </label>
